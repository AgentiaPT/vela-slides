...
> **v2.8.0** · 2026-03-25 · Compressed skill (-19%), fast-path loading, auto-discovery CLI reference
...
### Multi-step edit patterns

**Rebrand/restyle** (change colors, preserve everything else):
1. `deck list` → note slide count (must match after)
2. `deck replace-text deck.json "#oldHex" "#newHex"` for each color (auto-cascades to rgba)
3. `deck validate` → confirm same slide count, no errors

**Delete multiple slides** — always remove from **highest index first** to avoid re-indexing:
```bash
# To remove slides 5-10: remove 10, then 9, then 8... then 5
vela slide remove deck.json 10
vela slide remove deck.json 9
# ... etc
```

**Merge slides**: view both → write merged JSON to file → insert at position → remove originals (highest first).

**Translate**: `extract-text` → edit JSON map → `patch-text` (2 calls, not N×replace).

### Command selection
- **Find/replace**: `deck replace-text` | **Edit property**: `slide edit N key value` | **Edit block**: `slide edit N block.I.key value`
- **Structural**: `slide remove/move/duplicate` | **Regroup**: `deck split --sections "A:3,B:5"` or `--flat`
- **Inspect**: `deck list` (~200 tok) or `slide view N` (~150 tok) | **Preview**: add `--dry-run`
- **Full rewrites**: `slide view N --raw` → `str_replace` | **Deep nesting**: edit JSON directly
...
### Gotchas
- **`align: "center"` + SVG/divider/progress = invisible** — no intrinsic width. Use slide `align:"left"` + block `"align":"center"`, SVG needs `"maxWidth":"60%"`.
- **`$A` aliases must be defined in `C`** before use. Undefined = literal text.
- **Spacers are bare ints** — `8` not `{"_":"spacer","h":8}`.
- **Edits must preserve slide count** unless explicitly adding/removing. Always `deck list` before AND after to verify.
- **Rebrand = replace-text only** — never regenerate slides during a restyle. Just swap colors.

Icons: PascalCase Lucide names (1000+). Theming: see `references/themes.md`.
