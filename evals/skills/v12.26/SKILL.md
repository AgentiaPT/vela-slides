...
version: 12.26
updated: 2026-04-03
description: Create presentation decks using the Vela engine. Compact DSL format — never verbose JSON.
license: ELv2
compatibility: Requires Python 3 and Bash. Designed for Claude Code.
allowed-tools: Bash(python3 skills/vela-slides/scripts/*), Read, Write, Edit, Glob, Grep
effort: low
---
...
```bash
vela server start <folder-or-file> [--port 3030]      # local preview (default)
vela deck ship <deck.vela> --output <name.jsx>         # cloud artifact only
vela deck extract <source.jsx> <output.vela>           # extract from .jsx
```
`python3 skills/vela-slides/scripts/vela.py <resource> <action> [args...]`
When user asks to "load/show the demo": use `--demo`. Do NOT generate a new deck.
...
**`G`** — Sections: `[{"g":"Name","S":[slides]}]`. 3-5 narrative sections.
**Slide** — `{"t":"d","n":"Assertion Headline","d":60,"B":[blocks]}`. Cover/CTA: `bgGradient`,`align:"center"`,`verticalAlign:"center"`. Duration: cover 20, content 60-90, CTA 25.
