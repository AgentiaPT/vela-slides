...
- **Orchestrator objective: lowest total $/token cost over the sprint — measure it, don't just count turns.** Hub cost ≈ (standing context size) × (number of turns); nearly every token in a long sprint is a cache-read of context pinned earlier and re-billed every turn since. **≤50 main-loop turns is a heuristic for a normal sprint, not a hard ceiling** — a genuinely large/complex issue list legitimately needs more turns as cluster count grows. What must stay bounded is the standing context itself: keep the hub **thin AND flat** for the whole session (a payload pinned early is re-charged on every later turn), and checkpoint real spend mid-sprint with `assets/sprint-cost.py --audit`, course-correcting on cost/tokens, not on a turn number. The hub plans, delegates, merges, and reads verdicts — it does **not** debug, drive the browser, or write/run diagnostic scripts inline. Any diagnosis, capture, completeness-check, or fix-verification loop goes to a sub-agent that returns a compact verdict (`cov.cjs`/`imgdiag`/`capture`/`s13`-style probes are worker work, never hub work). **When a sub-agent dies mid-task (session/rate limit), spawn a REPLACEMENT sub-agent — never absorb its work into the hub** (the porter sprint hit 375 turns and $59.74 of hub cost — 72M cache-read tokens, 94% of its total — doing exactly this after an eval agent died). If you catch yourself on a 3rd inline Write+Bash probe on one question, stop and delegate the whole investigation. Rule of thumb: >2 consecutive inline diagnostic/driver round-trips = a missed delegation.
