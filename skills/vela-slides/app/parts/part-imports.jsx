...
const VELA_CHANGELOG = [
  { v: "12.80", d: "Local AI backend for the CLI: a new agent_backend.py routes Vera's AI calls to the local `claude` CLI (Claude Code print mode) over the existing loopback channel, so `vela server` can drive AI with no Anthropic API key. Mirrors the Neutralino gatekeeper's security posture — the agent runs as a pure text completion with every tool disabled. serve.py starts the channel automatically, and the offline render harness gained a --channel-port option so AI can be exercised end-to-end in a headless browser. No change to artifact runtime." },
