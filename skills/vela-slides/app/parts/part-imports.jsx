...
const VELA_CHANGELOG = [
  { v: "12.81", d: "Sprint 'Tradewinds' — share & present. Share: a new Export → Standalone HTML produces one shareable .html that opens as a clean read-only presentation (no editor chrome), self-transpiled and safely inlined, loading React/lucide from a CDN with SHA-pinned integrity; an optional 'Made with Vela ⛵' footer. Present: a dedicated presenter/speaker view (current slide + next-slide preview + speaker notes + live elapsed timer and per-slide budget), a grid gallery/overview now reachable from the editor (not just Present mode), and a tasteful deck-level slide transition on advance. One-prompt: Generate Deck from Source turns a pasted README / URL text / PDF text into a full deck in one shot over the existing AI path (visible-but-disabled when no AI is available). Present-mode polish: all edit affordances (selection outline, inline +, pencil) are fully suppressed while presenting so a shared deck shows only its content; the slide counter is larger and higher-contrast; block-level add affordances now hover-reveal consistently with slide-level ones; the AI-gated toolbar 'Edit' button is renamed 'AI Edit' so its disabled state no longer implies inline editing is off. Test honesty: the in-app UI battery selectors were realigned to the current DOM, AI-dependent battery tests skip-with-reason when AI is unavailable instead of failing, and the jsdom-gated node security suites skip cleanly (with a clear message) rather than counting as failures." },
