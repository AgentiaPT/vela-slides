...
const VELA_CHANGELOG = [
  { v: "12.54", d: "Security (Critical, audit 2026-06): fix </script> breakout in assemble.py. json.dumps(deck) was string-replaced into vela.jsx at the STARTUP_PATCH marker with no escape; the assembled .jsx is loaded inside <script type=\"text/babel\"> (app/local.html:392 and the Claude.ai artifact viewer), and HTML parsers close <script> on the literal </script token regardless of JS string quoting. A deck with `\"title\": \"</script><script>alert(1)</script>\"` therefore produced an out.jsx with the verbatim closing tag — full DOM/localStorage exfil and artifact-proxied API CSRF on render. The local-server pipeline (serve.py) already had this escape since the local-sync feature landed; assemble.py was the missing twin. Fix: deck_json_str.replace(\"</\", \"<\\\\/\") immediately after json.dumps, mirroring serve.py. CI regression test added at tests/test_vela.py asserting `</` does not appear unescaped in assembled output." },
