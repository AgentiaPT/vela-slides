...
**Image runs:** two or more adjacent `image` blocks auto-layout as a balanced grid; `imageCols` pins the column count.
...
**Layout note:** Left-aligned slides (`align: "left"` or default) stretch blocks to full width; center-aligned slides (`align: "center"`) shrink-wrap blocks. Flow, grid, and progress blocks therefore fill the canvas automatically on content slides.

**Cols layout:** When `layout: "cols"`, the `blocks` array renders full-width above the two columns (optional header area — badge, heading, etc.). `L`/`R` hold the left/right column blocks. At least one of L/R must be present. `contentFlex`/`imageFlex` control column width ratio (default 1:1); `splitGap` controls gap between columns (default 32px). All block types work inside L/R.

## Slide-level optional: `studyNotes` (offline student content, v12.32+)

Pre-authored companion content rendered in the 🎓 student panel with zero API calls — mirrors the live Vera Teacher output shape so the existing markdown + inline-SVG renderer is reused.
...
**Fields:**
- `text` *(required, string, ≤ 4000 chars, warn at 2000)* — markdown via the same `parseInline` + `ChatMarkdown` pipeline as live Vera chat. Supports `**bold**`, `*italic*`, `***both***`, `~~strike~~`, `__underscores__`, `[label](https://…)` external links, `[label](#term)` X-Ray glossary refs.
- `diagram` *(optional, string, ≤ 8000 chars)* — one complete, well-formed inline SVG, rendered via `sanitizeSvgMarkup` (DOMParser strip of `<script>`, event handlers, `javascript:/data:/vbscript:` URIs).
- `questions` *(optional, array, max 6, each ≤ 160 chars)* — pre-authored follow-ups. When the Claude API/MCP channel is reachable they render as clickable buttons feeding the live Vera Teacher flow; otherwise as static "Questions to ponder" bullets.
- `glossary` *(optional, object, max 24 entries)* — `{ termKey: { definition: string ≤ 400 chars, url?: sanitized URL } }`. Keys are lowercased, `[^\w-]` stripped on load (case-insensitive match). `[label](#term)` looks up the entry and shows a Kindle-style popover with definition + optional "Learn more" link; unknown terms render as plain text.
...
**Sanitization:**
- `text` strips HTML tags, NULL bytes (sentinel safety for the link parser), truncates at 4000 chars.
...
**Visible marker:** slides carrying `studyNotes` show a 🎓 badge in TOC, gallery thumbnails, and the main slide viewer (top-left, next to the top-right comment badge).

**Limitation:** X-Ray `[term](#key)` syntax activates only inside `studyNotes.text`. Regular text/heading/bullets/callout blocks call `parseInline(text)` without a glossary context, so it renders as plain label text there.

**Compact format:** short key `sN` on the slide object; value passed verbatim (the palette-alias pass skips `studyNotes` fields so literal hex codes in prose aren't mangled).
...
**Authoring:** JSON-only for v12.32. No in-app editor UI yet. A Vera `set_study_notes` tool is planned so the live engine can author offline notes on demand.
...
### code
Code snippet with label. Optional `copy: true` adds a "Copiar" clipboard-copy button.
```json
{
  "type": "code",
...
### grid
Multi-column layout; each cell holds its own blocks array.
...
### callout
Highlighted insight/tip box. Optional `reveal: true` makes it collapsible (starts closed).
```json
{
  "type": "callout",
...
### comparison
Side-by-side A vs B layout with semantic coloring, icon headers, optional central divider. Use for before/after, pros/cons, plan A vs B.
```json
{
  "type": "comparison",
  "items": [
...
### checklist
Status-aware item list with semantic icons/colors for each state: done, partial, pending, blocked.
```json
{
  "type": "checklist",
  "items": [
