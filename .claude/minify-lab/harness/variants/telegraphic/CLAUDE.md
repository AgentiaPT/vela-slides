...
App source (part-files): `src/parts/`. Build/preview/AI toolchain: `tools/vela-dev/` —
both dev-only, never shipped. **Lean skill** `skills/vela-slides/` carries only compiled
`vela.jsx` plus author→ship scripts (`vela.py`, `validate.py`, `assemble.py`).

**No bundler.** Python stdlib concatenation in fixed dependency order (~10ms).

### Part-File Order (fixed, never changes)

**`src/parts/MANIFEST.txt` is the single source of truth** for part list, build order,
each part's purpose — `concat.py`, `lint.py`, `tests/test_vela.py` all read it; none
keeps its own copy. Order is TDZ-sensitive (one shared scope, no modules): never
reorder. Add a new part there in the same change, or lint fails.
...
**Before your first code edit**: do the mandatory secure-coding read (see *Mandatory*
below — Triage sets how much; static chrome edits need only §0 + the helper table).
Post-edit hook reminds you once if skipped. Then open the named file(s) FIRST, grep
the named symbol — don't scan the tree.
Every row below is verified against the code (CI-enforced by `check-routing.py`);
if one is wrong, fix it in the same change. Line numbers deliberately omitted (they
...
dispatching parts (orphan actions flagged — check before assuming a UI exists),
every Vera tool, every UI-test suite. One grep there replaces a tree scan for any
change this table doesn't cover.
...
```
Author→ship only — shipped skill has no preview/AI backend (lives in
...
speculative polish. Found something else worth fixing? Note it for a
separate change — don't bundle it. (Measured: unscoped changes cost
~10% more agent time/tokens, harder to review.)
...
Vela renders untrusted deck JSON in runtimes with real filesystem/network
capability; nearly every vulnerability in this repo's history came from
ordinary feature code, not work labelled "security". Start with the
skill's **Triage** section: defines when §0 essentials + helper table
suffice (static app-chrome/editor-UI changes) vs. when full read is
...
Use the **`dependency-sweep`** skill — drives
`tools/vela-dev/scripts/dep-sweep.py` (deterministic discovery/cooldown/
provenance/audit checks, exit 4 = high/critical finding), owns judgement
calls. Not wired into CI by design.
...
- **One-off / interactive** work (explore, screenshot, reproduce, verify UX):
  **`playwright-cli-setup`** skill (persistent CLI browser, one command at a
  time). Default for manual tasks.
- **Repeatable, committed automation** (CI, benchmark, demo videos, headless
  UI battery, opt-in AI harness): **`vela-live-render`** skill
  (`render-offline.js` + `vela-drive.js`). AI mode strictly opt-in — spawns
  the user's `claude` under the lockdown flags shared with the Neutralino
  gatekeeper (parity-tested in `tests/test_serve.py`).
...
Covered by `playwright-cli-setup` skill (see section above): persistent
