...
list. The "where it bit" column names the *shape* of the mistake, not a target
map for an unpatched build; per the repo's disclosure discipline it stays at
class level. Read the commit for the full reasoning (the in-code comments carry
the durable, maintainer-facing version).
...
| 4 | Incomplete mediation | colour sinks gated at some renderers only; an export URI sink un-encoded; export text fields raw; a file-listing endpoint outside the shared containment gate | `cc97d13`, `af632bd`, `89444c4`, `88748e8` |
...
| 9 | Origin as a boundary | a local channel treated Origin as an access boundary rather than an unforgeable token; a bridge shipped without mandatory sender auth | `88748e8`, `e45159e` |
...
- **Adversarial re-review pays.** The markdown-export escaping chain and the
