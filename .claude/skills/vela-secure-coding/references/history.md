...
Each rule in `SKILL.md` §3 exists because this repo shipped it. Read the commit
for the full reasoning (the in-code comments carry the durable version).
Kept deliberately mechanism-light per the repo's disclosure discipline — the
commits and the in-code comments are the maintainer-facing detail.
...
| 6 | Incomplete escaping | Markdown export: inline-only link escaping → reference links → raw HTML/autolinks → backslash revive → table pipes; each round found the next form | `88748e8`, `4b486b5`, `e795100`, `89444c4`, `d189656` |
| 7 | Regex needing a well-formed match | `isSvgStyleSafe` required a closing paren; CSS comments split a function token | `6eb7804` |
