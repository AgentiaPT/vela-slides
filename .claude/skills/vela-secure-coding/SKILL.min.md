...
| Runtime | Worst case on bypass | Backstop |
|---|---|---|
| Neutralino desktop | file read/overwrite in the 2 allowed roots + outbound exfil (no host RCE — `os.spawnProcess` not granted) | `<meta>` CSP, enumerated `nativeAllowList`, `fs-guard`, deck-open warning |
...
Full detail (authoritative blast-radius table): `docs/SECURITY.md`,
`vela-neutralino/SECURITY.md`.
...
|---|---|---|
| Deck string → DOM/state | `sanitizeString(v, maxLen)` | own tag strip / `.replace(/<[^>]*>/g,"")` |
| Deck title | `sanitizeDeckTitle` | raw assignment |
| Link/URL | `sanitizeUrl(url)` — re-validate **at the sink** | returning raw input after validating a parsed view |
| `window.open` on deck link | `openExternalLink` | `window.open(url)` |
...
| New slide/block field | add to `SAFE_SLIDE_KEYS`/`SAFE_BLOCK_KEYS` (+ `validate.py`, `block-schema.md`, compact/turbo maps) | reading a key that isn't allowlisted (CI lint fails) |
...
| Marker substitution in template | `String.replace(marker, () => value)` (replacer **function**) | a string replacement (`$&`/`$1` splicing) |
...
- Every deck field you read is in `SAFE_BLOCK_KEYS`/`SAFE_SLIDE_KEYS` (key-drift
  lint enforces this) and is sanitized at ingress.
- Every colour/paint value reaching `background`, `backgroundImage`, `mask`,
  ...
  `cssUrl` (`check_css_fetch_sink_gate` lint enforces this).
...
- New field → allowlist entry + type + length/range clamp + right sanitizer.
