...
the premium context and biases the final gate). Three roles:
...
3. **Orchestrator, not worker — hub hygiene: payloads to disk, pointers + verdicts in the
   ...
   dies in an isolated window. *(Measured: one sprint's orchestrator was 65% of total spend,
   95% cache-reads; 10 hub-viewed screenshots were 31% of its cache-read and ~46% of its
   standing context — more than any other single bucket. Generally the single biggest cost
   lever, not specific to that run.)*
...
7. **Converge on a BLIND gate, not "one more final pass" — verify-each + hunt-across.**
   ...
   limit; every validator stays blind to sprint history regardless of scope. The biased main
   context declaring "done" is exactly the trap that spawns redundant "final" passes — see
   *Stop rule* and `references/orchestration.md` for the full protocol and the tradeoff (pure
