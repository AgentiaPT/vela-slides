...
Keep it high level; avoid including detail that helps reproduce the issue in the wild.
...
Avoid working payload / example attack string · exact bypass token or primitive · step-by-step repro · "where the gap was" map (precise unguarded field/endpoint/param an attacker should target) · chained CVE/exploit ref amounting to a recipe.
