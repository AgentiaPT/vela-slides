...
const VELA_CHANGELOG = [
  { v: "13.76", d: ["Decks: opening a deck no longer rewrites its lane and module ids.", "Validation: a gradient placed in a solid-colour field is now reported instead of dropped without a warning.", "Vector PDF: currency and typographic characters now use the real font glyph metrics.", "Editor: blocks can be copied and pasted, with an insert marker shown before the paste commits.", "Editor: the block toolbar stays inside the slide on a full-bleed block, so it can still be clicked.", "Text: an inline link mark now stays on its own label line instead of drifting to the right.", "Review: new editor review mode — mark a slide as approved and it leaves the editor cycle. Presenter mode still shows every slide.", "Slide list: slide rows now have a delete control, like sections.", "Gallery: slide rows now have a hide control beside delete.", "Chrome: the gallery and presenter edit icons now have their own backdrop, so they stay visible over any slide.", "Views: an Editor and Gallery switcher now sits beside Present.", "Branding: an accent-bar height of 0 now removes the bar completely.", "Branding: the branding settings moved to a right-hand side pane with larger, readable controls.", "Desktop: the window title now carries the deck title, and the keyboard responds as soon as the window regains focus.", "Desktop: the AI integration now connects on the first start after a new build.", "Desktop: the standalone HTML export no longer fails with a duplicate declaration."] },
  { v: "13.75", d: ["Tests: repaired a UI-battery suite that a merge left incomplete.", "Build: the build now parses the bundle and names the part that has a syntax error."] },
