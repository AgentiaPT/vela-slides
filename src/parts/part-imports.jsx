...
const VELA_CHANGELOG = [
  { v: "13.46", d: ["fix: palette keys without the $ sigil no longer silently discard the whole palette (normalised with a warning)", "fix: unresolved colour aliases now fail the build instead of shipping; --allow-unresolved to override", "fix: alias substitution is exact-token and prose subtrees (incl. studyNotes) are exempt at every depth", "fix: ambiguous or malformed palette keys are rejected instead of silently colliding (key grammar: $ + 1-2 letters)", "fix: full-format decks with a top-level 'C' palette now resolve it; standalone validate.py expands G-form decks and honours --allow-unresolved", "security: validate.py now fails closed instead of silently skipping its colour-alias check if its own palette-resolution helpers can't load", "security: abnormally deep decks now fail cleanly during palette expansion instead of an uncaught recursion error"] },
