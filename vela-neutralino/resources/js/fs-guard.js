...
//   2. Confinement (async, at the points where a path enters trust or is about
//      to be read or written). Lexical containment alone believes the path
//      string: an entry inside a root that leads elsewhere satisfies every
//      string test while the OS reads or writes somewhere else entirely, and
//      nothing in the Neutralino client API can tell the difference from in
//      here (no realpath; getStats reports neither links nor reparse points).
//      allowVerified()/inspect()/assertSafe() ask the gatekeeper extension,
//      which answers using os.Root — the standard library's traversal-resistant
//      file API — and callers fail closed on the answer.
//
...
import { askGatekeeper } from "./resolve-bridge.js";

const roots = [];          // normalized absolute roots, no trailing slash
let installed = false;

// SECURITY: the confinement bridge is a STATIC import, not something handed in
// at boot. A module binding cannot be reassigned at all, so script that reaches
// the realm later — by definition through deck content — has nothing to swap
// out. An injection point would only be as good as the one-shot guard written
// around it; this needs no guard.
...
// Longest registered root containing `n`. Confinement is always evaluated
// relative to a root, so every check needs to know which one a path belongs to
// — and the name sent across the wire is relative to it.
function rootFor(n) {
  let best = "";
...
export const fsGuard = {
  ...
  // Register a root, having first asked where it physically lives.
  //
  // A folder the user deliberately points elsewhere ("~/Decks → ~/Dropbox/
  // Decks") keeps working: both the name and its physical form are registered,
  // because the app addresses decks by the name the user picked. Each form
  // faces the same volume/shallow/system-root refusals, so a folder that is
  // itself a link cannot become a way into a location we would never accept as
  // a root.
  //
  // Returns { ok, degraded }. `degraded` means the root is registered but its
  // physical location could not be established — confinement of everything
  // inside it still applies, so this is a weaker guarantee, not an absent one.
...
async function inspect(paths) {
  ...
  // Lexical containment first — no round trip, and it is the check that has
  // always been here. Anything it rejects never reaches the gatekeeper.
  const byRoot = new Map();
  for (const n of list) {
    const root = underRoot(n) ? rootFor(n) : "";
    if (!root) { out.set(n, "unclean"); continue; }
    if (!byRoot.has(root)) byRoot.set(root, []);
    byRoot.get(root).push(n);
  }

  // One gatekeeper call per root: it opens the root once and checks the whole
  // batch against it, which is what makes vetting a folder listing cheap.
  for (const [root, group] of byRoot) {
    // "." is the root itself; os.Root takes root-relative names.
    const names = group.map((n) => (n === root ? "." : n.slice(root.length + 1)));
    const res = await askGatekeeper({ root, names });
    group.forEach((n, i) => {
      const entry = res && res.entries ? res.entries[names[i]] : null;
      if (!entry) out.set(n, "degraded");
      else if (entry.ok) out.set(n, "clean");
      else out.set(n, entry.error === "missing" ? "missing" : "unclean");
    });
  }
  return out;
}
...
async function assertSafe(path) {
  ...
  if (verdict === "unclean") {
    throw new Error(`[fs-guard] refusing ${norm(path)}: it leads outside its folder`);
  }
  return verdict;
}
...
// swapping `install`). The allowlist, the `installed` flag and the guard
// helpers live in module-private closure state that the frozen surface never
// exposes; the confinement bridge is a static import, which cannot be
// reassigned at all.
