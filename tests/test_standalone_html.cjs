 ...
 *
 * Covers buildStandaloneHtml() and its helpers (part-export-md.jsx), which power the
 ...
 *
 * This loads the REAL functions from part-export-md.jsx (regex-extracted between the
...
const PDF_SRC_PATH = path.join(REPO, "src/parts/part-export-md.jsx");
...
// ── 6 (meridian-CR12): Neutralino source already carries sync-vela.py's UMD shim ──
// Neutralino's getStandaloneJsxSource() fetches resources/vela.jsx, which
// sync-vela.py prefixed with the same React/lucide destructure this builder
// adds. Build from the REAL sync-vela.py preprocess() output and prove the
// inlined script declares the hooks once and compiles as a classic script.
{
  const { execFileSync } = require("child_process");
  let shimmed = null;
  try {
    shimmed = execFileSync("python3", ["-c",
      "import importlib.util,sys\n" +
      "spec=importlib.util.spec_from_file_location('sv', sys.argv[1]); m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m)\n" +
      "sys.stdout.write(m.preprocess(open(sys.argv[2],encoding='utf-8').read()))",
      path.join(REPO, "vela-neutralino/scripts/sync-vela.py"), VELA_JSX_PATH],
      { encoding: "utf8", maxBuffer: 64 * 1024 * 1024 });
  } catch (e) { check("sync-vela.py preprocess() runs", false, e.message); }
  if (shimmed) {
    check("CR12 input really starts with the sync-vela.py shim (test not vacuous)", shimmed.startsWith("// --- Neutralino UMD shim (generated by sync-vela.py)"));
    let nlHtml = null;
    try { nlHtml = buildStandaloneHtml(shimmed, deck, { footer: false, babel: Babel }); }
    catch (e) { check("CR12 buildStandaloneHtml accepts a sync-vela.py-shimmed source", false, e.message); }
    if (nlHtml) {
      const code = extractAppScriptContent(nlHtml);
      // The app's own source also holds this list as a string literal (the
      // builder's shim text), so compare with the pristine-source build: the
      // shimmed build must add ZERO extra hook destructures.
      const count = (t) => (t.match(/useState,\s*useReducer,\s*useEffect/g) || []).length;
      const decls = count(code), base = count(extractAppScriptContent(htmlNoFooter));
      check("CR12 one shim only: shimmed source adds no second hooks destructure", decls === base, `shimmed ${decls} vs pristine ${base}`);
      let compileErr = null;
      try { new vm.Script(code); } catch (e) { compileErr = e.message; }
      check("CR12 inlined script compiles (no duplicate declaration)", compileErr === null, compileErr);
      check("CR12 deck still spliced after the shim strip", code.includes(deck.deckTitle));
    }
  }
  // The strip is anchored at offset 0: a shim-looking comment later in the source is kept.
  const notAtStart = "const a = 1;\n// --- Neutralino UMD shim (generated by sync-vela.py) ---\nconst b = 2;\n// ----------\n\nconst STARTUP_PATCH = null;\nconst VELA_PRESENTATION_MODE = false;\nfunction App(){return null}\n";
  const htmlKeep = buildStandaloneHtml(notAtStart, { deckTitle: "K", lanes: [] }, { babel: Babel });
  check("CR12 shim strip only acts at the start of the template", /\bb\s*=\s*2/.test(extractAppScriptContent(htmlKeep)));

  // D1: a shim written with CRLF line ends (e.g. a Windows-side sync-vela.py
  // run before the newline="\n" fix) must still be stripped, not just an
  // LF-written one.
  if (shimmed) {
    const shimmedCrlf = shimmed.replace(/\n/g, "\r\n");
    let nlHtmlCrlf = null;
    try { nlHtmlCrlf = buildStandaloneHtml(shimmedCrlf, deck, { footer: false, babel: Babel }); }
    catch (e) { check("D1 buildStandaloneHtml accepts a CRLF-shimmed source", false, e.message); }
    if (nlHtmlCrlf) {
      const codeCrlf = extractAppScriptContent(nlHtmlCrlf);
      const count = (t) => (t.match(/useState,\s*useReducer,\s*useEffect/g) || []).length;
      const declsCrlf = count(codeCrlf), base = count(extractAppScriptContent(htmlNoFooter));
      check("D1 CRLF shim strip: no second hooks destructure", declsCrlf === base, `crlf ${declsCrlf} vs pristine ${base}`);
      let compileErrCrlf = null;
      try { new vm.Script(codeCrlf); } catch (e) { compileErrCrlf = e.message; }
      check("D1 CRLF-shimmed inlined script compiles (no duplicate declaration)", compileErrCrlf === null, compileErrCrlf);
    }
  }
}
