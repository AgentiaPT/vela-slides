...
// Neutralino runtime source is already prefixed with its UMD shim. Exporting
// from that surface must strip the old shim before adding the standalone one.
const neutralinoShim = `// --- Neutralino UMD shim (generated by sync-vela.py) ------------------
...
let neutralinoHtml;
try {
  neutralinoHtml = buildStandaloneHtml(neutralinoShim + jsxSource, deck, { footer: false, babel: Babel });
} catch (e) {
  console.error("Neutralino-source standalone export threw:", e.stack || e.message);
  process.exit(1);
}
check("Neutralino-source export compiles without duplicate React declarations", typeof neutralinoHtml === "string" && neutralinoHtml.length > 10000);
