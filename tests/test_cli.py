...
def _check_package_skill(tmpdir):
    ...
    # (3) Build artifacts excluded: __pycache__ dirs and *.pyc/*.pyo files must
    #     never enter the archive. The release workflow packages a tree in which
    #     the test suite has already generated bytecode caches, so this is the
    #     property that keeps them out of the shipped skill ZIP.
    proot = os.path.join(tmpdir, "pycparent", "skill")
    os.makedirs(os.path.join(proot, "scripts", "__pycache__"))
    with open(os.path.join(proot, "SKILL.md"), "w") as f:
        f.write("keep me")
    with open(os.path.join(proot, "scripts", "tool.py"), "w") as f:
        f.write("# keep")
    with open(os.path.join(proot, "scripts", "__pycache__", "tool.cpython-312.pyc"), "wb") as f:
        f.write(b"\x00bytecode")
    with open(os.path.join(proot, "scripts", "stray.pyc"), "wb") as f:
        f.write(b"\x00stray")
    pyc_zip = os.path.join(tmpdir, "pyc.zip")
    package_skill.build_zip(proot, pyc_zip)
    with zipfile.ZipFile(pyc_zip) as zf:
        pnames = zf.namelist()
    check(any(n.endswith("SKILL.md") for n in pnames)
          and any(n.endswith("scripts/tool.py") for n in pnames)
          and not any("__pycache__" in n for n in pnames)
          and not any(n.endswith((".pyc", ".pyo")) for n in pnames),
          "package-skill excludes __pycache__ dirs and *.pyc/*.pyo files")
