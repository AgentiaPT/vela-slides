...
Vela Slides is a **single-file React application** (15,810 lines, ~1.2 MB) designed to run inside Claude.ai's artifact sandbox. The sandbox requires all code to be in one `.jsx` file with no external module imports between files — so Vela uses a **modular source / concatenated output** architecture.
...
|---|---|---|
| `part-imports.jsx` | ~1,343 | Constants (FONT, SIZES, COLORS), deck sanitization, import/export helpers, storage API, Levenshtein matching, startup patch system |
| `part-icons.jsx` | ~291 | `getIcon()` resolver with 270+ Lucide icon mappings, aliases, emoji fallback |
| `part-blocks.jsx` | ~1,696 | Every block renderer: heading, text, bullets, flow, grid, metric, timeline, steps, table, callout, quote, SVG, badge, icon, icon-row, tag-group, progress, code, image, divider, spacer, comparison, funnel, cycle, number-row, matrix, checklist. Plus `EditableText` for WYSIWYG. |
...
| `part-slides.jsx` | ~2,255 | `SlidePanel` component, slide rendering pipeline, fullscreen presenter, branding overlay, thumbnail generation, image compression |
...
| `part-test.jsx` | ~333 | `VelaBatteryTest` — automated render tests for block types |
...
| `part-app.jsx` | ~1,539 | `VelaApp` root component, modals (JSON clipboard, shortcuts, changelog), keyboard handlers, mobile navigation, file browser |
