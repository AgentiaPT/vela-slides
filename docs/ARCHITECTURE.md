...
Vela Slides is a **single-file React application** (19,086 lines, ~1.3 MB) designed to run inside Claude.ai's artifact sandbox. The sandbox requires all code to be in one `.jsx` file with no external module imports between files — so Vela uses a **modular source / concatenated output** architecture.
...
part-uitest     → UI integration tests (206 tests in 39 suites)
...
|---|---|---|
| `part-imports.jsx` | ~1,520 | Constants (FONT, SIZES, COLORS), deck sanitization, import/export helpers, storage API, Levenshtein matching, startup patch system |
...
| `part-list.jsx` | ~660 | `ModuleList`, `LaneSection`, `ConceptRow`, drag-and-drop reordering, AI slide adder |
...
| `part-uitest.jsx` | ~2,130 | 206 UI integration tests in 39 suites — comprehensive coverage of block rendering, themes, edge cases |
