...
Vela Slides is a **single-file React application** (15,144 lines, ~1.2MB) designed to run inside Claude.ai's artifact sandbox. The sandbox requires all code to be in one `.jsx` file with no external module imports between files — so Vela uses a **modular source / concatenated output** architecture.
...
part-uitest     → UI integration tests (159 tests in 26 suites)
...
|---|---|---|
| `part-imports.jsx` | ~1,296 | Constants (FONT, SIZES, COLORS), deck sanitization, import/export helpers, storage API, Levenshtein matching, startup patch system |
| `part-icons.jsx` | ~189 | `getIcon()` resolver with 270+ Lucide icon mappings, aliases, emoji fallback |
| `part-blocks.jsx` | ~1,383 | Every block renderer: heading, text, bullets, flow, grid, metric, timeline, steps, table, callout, quote, SVG, badge, icon-row, tag-group, progress, code, image, divider, spacer. Plus `EditableText` for WYSIWYG. |
| `part-reducer.jsx` | ~268 | `useReducer` state shape, all dispatch actions (SELECT, LOAD, ADD_LANE, SET_SLIDES, etc.) |
| `part-engine.jsx` | ~1,176 | `callClaudeAPI()`, Vera system prompts, tool definitions, slide improve/edit/create/alternatives, batch operations, agentic ReAct loop |
| `part-slides.jsx` | ~2,163 | `SlidePanel` component, slide rendering pipeline, fullscreen presenter, branding overlay, thumbnail generation, image compression |
| `part-list.jsx` | ~422 | `ModuleList`, `LaneSection`, `ConceptRow`, drag-and-drop reordering, AI slide adder |
| `part-chat.jsx` | ~437 | `ChatPanel`, message rendering, tool trace cards, image paste/drop, starter prompts |
| `part-test.jsx` | ~244 | `VelaBatteryTest` — automated render tests for block types |
| `part-uitest.jsx` | ~1,538 | 159 UI integration tests in 26 suites — comprehensive coverage of block rendering, themes, edge cases |
| `part-demo.jsx` | ~861 | Cinematic demo mode with 18 scenes showcasing all Vela features |
| `part-pdf.jsx` | ~3,651 | Canvas-based PDF renderer, watermark system, link annotations, markdown export |
| `part-app.jsx` | ~1,516 | `VelaApp` root component, modals (JSON clipboard, shortcuts, changelog), keyboard handlers, mobile navigation, file browser |
