...
Vela Slides is a **single-file React application** (20,948 lines, ~1.5 MB) designed to run inside Claude.ai's artifact sandbox. The sandbox requires all code to be in one `.jsx` file with no external module imports between files — so Vela uses a **modular source / concatenated output** architecture.
...
part-uitest     → UI integration tests (233 tests in 42 suites)
...
| `part-blocks.jsx` | ~1,910 | Every block renderer: heading, text, bullets, flow, grid, metric, timeline, steps, table, callout, quote, SVG, badge, icon, icon-row, tag-group, progress, code, image, comparison, funnel, cycle, number-row, matrix, checklist, divider, spacer. Plus `EditableText` for WYSIWYG. |
| `part-reducer.jsx` | ~385 | `useReducer` state shape, all dispatch actions (SELECT, LOAD, ADD_LANE, SET_SLIDES, etc.) |
...
| `part-test.jsx` | ~405 | `VelaBatteryTest` — automated render tests for block types |
| `part-uitest.jsx` | ~2,800 | 233 UI integration tests in 42 suites — comprehensive coverage of block rendering, themes, edge cases |
| `part-demo.jsx` | ~860 | Cinematic demo mode with 18 scenes showcasing all Vela features |
| `part-pdf.jsx` | ~4,050 | Canvas-based PDF renderer (raster + vector export path), watermark system, link annotations, markdown export |
