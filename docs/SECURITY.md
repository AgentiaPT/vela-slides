...
| Token at rest | The token is **not** persisted. It reaches the browser in the launch URL, or the operator supplies it via `VELA_TOKEN` / `--token`. `.vela.env` holds discovery data only (pid, port, host, mode) — nothing a local process cannot already read from `ps`/`netstat`. `--token-file` opts in to a separate `.vela.token`, written only through the canonical helper, which proves the file is owner-only before the token reaches disk and writes nothing when it cannot. Windows and WSL `drvfs` are refused outright — POSIX mode bits do not restrict a file there — so on those the token stays unpersisted and `VELA_TOKEN` is the supported route |
...
|------------|-------------|------|
| Mirochill | XSS Vuln., Beacon Vuln., Origin Checks, SymLink| 2026-May, 2026-Jul, 2026-Aug |
