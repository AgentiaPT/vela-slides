...
├── part-imports.jsx    # ~1940L — constants, sanitizers, helpers, storage
├── part-icons.jsx      #  ~290L — icon resolution system
├── part-blocks.jsx     # ~1910L — all 27 block renderers, EditableText
├── part-reducer.jsx    #  ~385L — state management, dispatch actions
├── part-engine.jsx     # ~1240L — AI tools, system prompts, slide ops
├── part-slides.jsx     # ~2580L — SlidePanel, Fullscreen, Branding
├── part-list.jsx       #  ~780L — Lane, ConceptRow, AI adder
├── part-chat.jsx       #  ~450L — ChatPanel, tool trace cards
├── part-test.jsx       #  ~405L — VelaBatteryTest
├── part-uitest.jsx     # ~2800L — 233 UI tests in 42 suites
├── part-demo.jsx       #  ~860L — Cinematic demo mode (18 scenes)
├── part-pdf.jsx        # ~4050L — PDF export (raster + vector), markdown export
├── part-pptx.jsx       # ~1190L — native editable PowerPoint (.pptx) export
└── part-app.jsx        # ~2070L — main VelaApp component
...
The Vela app is ~20,948 lines / ~1.5MB. Editing a monolith that large carries real risks:

| Concern | Monolith | Modular Parts |
|---|---|---|
| str_replace collision risk | High — many similar patterns across 20,948L | Low — part files are ~290–4050L |
| Context needed to edit | Must read entire 1.5MB file | Read only relevant part |
...
| Finding what to edit | Grep through 20,948 lines | File name tells you where to look |
...
```bash
python3 tests/test_vela.py          # 503 tests
python3 -m unittest tests.test_serve # 124 server tests
```
