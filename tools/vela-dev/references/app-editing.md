...
├── part-imports.jsx    # ~1550L — constants, sanitizers, helpers, storage
├── part-icons.jsx      #  ~290L — icon resolution system
├── part-blocks.jsx     # ~1890L — all 27 block renderers, EditableText
├── part-reducer.jsx    #  ~390L — state management, dispatch actions
├── part-engine.jsx     # ~1240L — AI tools, system prompts, slide ops
├── part-slides.jsx     # ~2560L — SlidePanel, Fullscreen, Branding
├── part-list.jsx       #  ~750L — Lane, ConceptRow, AI adder
├── part-chat.jsx       #  ~450L — ChatPanel, tool trace cards
├── part-test.jsx       #  ~360L — VelaBatteryTest
├── part-uitest.jsx     # ~2630L — 229 UI tests in 42 suites
...
└── part-app.jsx        # ~2010L — main VelaApp component
...
The Vela app is ~20,119 lines / ~1.4MB. Editing a monolith that large carries real risks:

| Concern | Monolith | Modular Parts |
|---|---|---|
| str_replace collision risk | High — many similar patterns across 20,119L | Low — part files are ~290–3960L |
...
| Finding what to edit | Grep through 20,119 lines | File name tells you where to look |
...
```bash
python3 tests/test_vela.py          # 421 tests
