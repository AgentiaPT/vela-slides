...
AI-native presentation engine for Claude.ai. Single-file React app (~1.5MB, 20,948 lines) that runs inside Claude.ai artifacts. Users describe slides in conversation, Vela renders them with 27 semantic block types.
...
| `part-uitest.jsx` | 233 UI tests in 42 suites |
...
`window.__velaRunUITests()`. Never inline the 1.5MB monolith as `text/babel` (its XSS-test
