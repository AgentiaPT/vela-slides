...
>
> Every line of Vela — 19,000+ lines of JSX, Python CLI tools, and build scripts — was generated entirely by AI (Claude). The codebase is validated by extensive AI code reviews, 500+ automated tests, and static analysis, but no human has ever read or audited the code.
>
...
1. Download **[`vela-slides-skill-v*.zip`](https://github.com/AgentiaPT/vela-slides/releases/latest)** from the latest release (or build a fresh one: `python3 tools/vela-dev/scripts/package-skill.py`)
...
├── tests/
│   ├── test_vela.py           ← Core engine tests (410 tests)
│   └── test_serve.py          ← Server endpoint & security tests (121 tests)
