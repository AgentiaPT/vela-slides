...
**No bundler.** Python stdlib concatenation, fixed dependency order (~10ms).
...
Per-part purpose: `src/parts/MANIFEST.txt`. Size/section map (banner sections w/ line/byte/token counts, to find a region without reading it all):
...
|---|---|---|
| Block renderer (new block type/layout) | `part-blocks.jsx` | `RenderBlock` switch; defaults `newItemFor`/`blankItemFor`/`PLACEHOLDER_FIELDS`. New deck field also needs `SAFE_BLOCK_KEYS`, `validate.py`, `references/block-schema.md` |
| Sanitization / allowlists / new slide or block field | `part-imports.jsx` | `SAFE_SLIDE_KEYS`, `SAFE_BLOCK_KEYS`, `sanitizeBlock`, `sanitizeSlide`, `validateAndSanitizeDeck`, `cssColor`/`cssGradient`/`cssUrl`, `sanitizeStyle`; reload path `resanitizeLoadedLanes`/`resanitizeLoadedBranding`. Read `.claude/skills/vela-secure-coding/SKILL.md` first |
| Slide chrome — accent bar, footer, slide numbers, logo | `part-branding.jsx` | `BrandingOverlay` |
| Lane/module/slide list (TOC), per-row controls | `part-list.jsx` | `ModuleList` → `ConceptRow` (caret, ▲/▼, `REORDER`) → `SlideListWithAdder`; menus `ContextMenu`/`CtxItem`/`AddMenu`; `AiSlideAdder` |
| Block toolbar & in-slide editing UX | `part-canvas.jsx` | `renderBlockItem` (hover toolbar + AI popup) in `SlideContent`; inline text `EditableText`/`ItemChrome` stay in `part-blocks.jsx`; AI driver `runBlockEdit` in `part-slidepanel.jsx` |
| Fullscreen/presenter/gallery | `part-slidepanel.jsx` (`SlidePanel`: nav, Fullscreen-API sync), `part-slides.jsx` (`PresenterView`, TOC, Gallery, `StudentPanel`) | `SlidePanel` renders others; state via `SET_FULLSCREEN` in `part-reducer.jsx` |
...
| PPTX export | `part-pptx.jsx` | `buildPptx`, `pptxEsc`, `pptxExtractTextBoxes` (reuses pdf-extract); modal `PptxExportModal` in `part-app-modals.jsx` |
| App-level modals — PPTX/help/stats/changelog/comments/new-deck/merge-patch/AI cost & agent status | `part-app-modals.jsx` | `PptxExportModal`, `StatsDialog`, `ChangelogDialog`, `CommentsPanel`, `NewDeckDialog`, `ShortcutHelp`, `CostBadge`, `AgentStatusChip`, `AgentSettingsDialog`, `MergePatchDialog`, `ModalBackdrop` — rendered by `App` in `part-app.jsx` (concatenates immediately after) |
| Markdown export | `part-export-md.jsx` | `deckToMarkdown`, encoders `mdInline`/`mdCell`/`escGap`; standalone-HTML export same file |
| Vera AI engine tools | `part-engine.jsx` | `executeTool` switch AND tool-contract prose in the system prompt, same file. Both must change together or the model calls a tool that doesn't exist |
| Reducer action / undo-redo | `part-reducer.jsx` | `innerReducer` (action switch), `NO_HISTORY` (actions that must NOT create an undo step), `reducer` (history wrapper), `MAX_HISTORY` |
...
Virtual canvas **960×540px** (16:9). Deck JSON has three interchangeable formats — Full (named keys), Compact (~32% smaller), Turbo (~47% smaller, positional) — auto-expanded on load. 27 block types across text, lists, data, flow, media, layout groups.
...
```
Author→ship only (shipped skill has no preview/AI backend — that lives in `tools/vela-dev/`). Exit codes: 0 ok, 1 fail, 2 usage, 3 not-found, 4 validation, 5 conflict. Supports `--json` and `--dry-run`.
...
Direct HTTP to the Anthropic API via the artifact proxy (no client key). ReAct loop, 22 tools, model `claude-sonnet-4-20250514`, temp 0, 16K max tokens, session cost tracking. All in `src/parts/part-engine.jsx`.
...
- **One-off / interactive** work (explore, screenshot, reproduce, verify UX): use the **`playwright-cli-setup`** skill (persistent CLI browser, one command at a time). Default choice for manual tasks.
- **Repeatable, committed automation** (CI, benchmark, demo videos, headless UI battery, opt-in AI harness): use the **`vela-live-render`** skill (`render-offline.js` + `vela-drive.js`). AI mode is strictly opt-in — it spawns the user's `claude` under the lockdown flags shared with the Neutralino gatekeeper (parity-tested in `tests/test_serve.py`).
- Browser-truth security checks (does a payload actually fire?): skill **`vela-browser-test`**.
